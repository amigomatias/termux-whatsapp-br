...
const vcard = 'BEGIN:VCARD\n' // metadata of the contact card
            ...
            + 'TEL;type=CELL;type=VOICE;waid=5598987290587:+55 (98) 98729-0587\n' // WhatsApp ID + phone number
            + 'END:VCARD'
prefix = '.'
blocked = []
limitawal = '50'
cr = '*Número Verificado*'
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				...
				levelnoton: '❬ X ❭ *a nota de nivelamento está ativa*',
				...
				only: {
					...
					Badmin: '[❗] Este comando só pode ser usado quando o bot se torna administrador! ❌',
                                        daftarB: `──「AINDA NÃO REGISTRADO」──\nOlá mano!\nVocê ainda não se cadastrou, vamos se cadastrar primeiro ... \n\nComando : ${prefix}register nome|idade\nExemplo : ${prefix}register matias|18`,
				}
			}
    			const apakah = ['sim','Não']
        		const bisakah = ['Você pode','Não pode']
		        const kapankah = ['Hari Lagi','Minggu Lagi','Bulan Lagi','Tahun Lagi']
			const botNumber = client.user.jid
			const ownerNumber = ["559887290587@s.whatsapp.net"] // replace this with your number
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        const NomerOwner = '559887290587@s.whatsapp.net'
				...
				case 'demote':
					...
					if (mentioned.length > 1) {
						...
						for (let _ of mentioned) {
							teks += `Pedido realizado, removendo da posição de administrador:\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					} else {
						mentions(` *Rebaixado* @${mentioned[0].split('@')[0]}\n de administrador. _*${groupMetadata.subject}*_`, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					}
					break
                                case 'randomhentai':
    ...
    client.sendMessage(from, options, text, { quoted: mek } )
			...
			case 'quotes':
              ...
              case 'owner':
               ...
               client.sendMessage(from, 'Aqui está o número do meu dono, mano, salve mano, salve depois',MessageType.text, { quoted: mek} )
                break
	case 'hidetag':
                client.updatePresence(from, Presence.composing) 
                if (!isRegister) return reply(mess.only.daftarB)
               ...
               if (data.error) return reply(data.error)
				...
				case 'adminlist':
					client.updatePresence(from, Presence.composing) 
                                        if (!isRegister) return reply(mess.only.daftarB)
					if (!isGroup) return reply(mess.only.group)
					teks = `Listando adm's do grupo *${groupMetadata.subject}*\nTotal : ${groupAdmins.length}\n\n`
					no = 0
				...
				case 'alay':
                  ...
                  if (!isOwner) return reply(mess.only.ownerB)
					...
					client.groupLeave (from) 
                     ...
                     setTimeout( () => {
					client.updatePresence(from, Presence.composing) 
					client.sendMessage(from, 'Sord é melhor... flw!', text) // ur cods
					}, 0)
                     break
          ...
          contextInfo: { mentionedJid: [from] }
           ...
           break
				...
				case 'add':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args.length < 1) return reply('Você deseja adicionar alguém?')
				...
				case 'kick':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Mencione o alvo antes!')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'Não vem chorar no pv do adm kkk\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
						...
						client.groupRemove(from, mentioned)
					client.sendMessage(mentioned, 'Foi kickado😂', text)
					}
					break
				case 'exe':
	              client.updatePresence(from, Presence.composing) 
				...
				case 'ocr':
                    ...
                    reply('Masalah telah di laporkan ke owner BOT, laporan palsu/main2 Não akan ditanggapi.')
                    break
               case 'apakah':
               client.updatePresence(from, Presence.composing) 
	    ...
	    case 'kapankah':
			...
			case 'closegc':
				...
				case 'toimg':
				    client.updatePresence(from, Presence.composing)
                                    if (!isRegister) return reply(mess.only.daftarB)
					if (!isQuotedSticker) return reply('❌ apenas figurinhas de foto❌')
					reply(mess.wait)
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						fs.unlinkSync(media)
						if (err) return reply('❌ falha ao converter ❌')
                ...
                break
                   ...
                   break
					...
					if (Number(args[0]) === 1) {
						...
						reply('❬ sucesso! ❭ mengaktifkan fitur welcome di group ini')
					} else if (Number(args[0]) === 0) {
						welkom.splice(from, disable)
						fs.writeFileSync('./database/json/welkom.json', JSON.stringify(welkom))
						reply('❬ sucesso! ❭ menonaktifkan fitur welcome di group ini')
					} else {
					...
					break
                                ...
                                case 'event':
                                        ...
                                        if (Number(args[0]) === 1) {
                                                ...
                                                reply('*❬ sucesso! ❭ Ativado EVENT neste grupo*')
                                        } else if (Number(args[0]) === 0) {
                                                event.splice(from, 1)
                                                fs.writeFileSync('./database/json/event.json', JSON.stringify(event))
                                                reply('*❬ sucesso! ❭ Desativado EVENT neste grupo*')
                                        } else {
                                                reply(ind.satukos())
                                        }
                                        break
                                case 'register':
                                        if (isRegister) return  reply(`「 JÁ REGISTRADO 」 \n\nO irmão registrou sis...`)
                                ...
                                case 'mining':
                                        ...
                                        if (!isEventon) return reply(`maaf ${pushname} event mining Não di aktifkan oleh owner`)
                                        if (isOwner) {
                                                const one = 999999999
